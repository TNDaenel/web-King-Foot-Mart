...
import { Button, Input, Popconfirm, Spin, notification } from 'antd'
...
export default function ListNews() {
  ...
  const [news, setNews] = useState(tintucData)
  ...
  const onhandleSearch = (value: string) => {
    const data = tintucData?.filter((item) => item.tieude.toLowerCase().includes(value.toLowerCase()))
    setNews(data)
  }
      ...
      {isLoading ? (
        ...
        <div>
          <header className='flex items-center justify-between mb-4'>
            ...
            <Input.Search placeholder='Search news' onSearch={onhandleSearch} style={{ width: 600 }} />
                    ...
                    <tr key={index}>
                      ...
                      <td className='whitespace-nowrap px-4 py-2 text-gray-700'>
                        <p className='text-xs lg:text-base md:text-xl mx-10'>
                          {tintuc.trang_thai === 'active' ? 'Công khai' : 'Riêng tư'}
                        </p>
                      </td>
                      <td className='whitespace-nowrap px-4 py-2'>
