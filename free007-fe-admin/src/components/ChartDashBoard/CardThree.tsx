...
const CardThree = ({ data, data2 }: CardThreeProps) => {
  ...
  const listStatusOrder = ['Chờ xác nhận', 'Xác nhận đơn hàng', 'Hoàn thành đơn hàng', 'Hủy đơn hàng']
  const dataPieChart =
    data &&
    data.countOrderStatus.map((orderStatus, index) => ({
      name: listStatusOrder[index],
      value: orderStatus.value
    }))
  const dataPieChart2 =
    data &&
    data.countOrderStatus.map((_, index) => ({
      name: listStatusOrder[index],
      value: data?.moneyOrderStatus[index].value
    }))

  const COLORS = ['#0088FE', '#FFBB28', '#00C49F', '#c0392b']
      ...
      <div className='grid grid-cols-4 gap-5 w-full'>
        <div className='flex col-span-4'>
          <div className='w-full flex flex-col items-center'>
            <span className='font-semibold text-lg'>Thống kê số lượng đơn hàng theo trạng thái</span>
          ...
          <div className='w-full flex flex-col items-center'>
            <span className='font-semibold text-lg'>Thống kê doanh thu đơn hàng theo trạng thái</span>
            <ResponsiveContainer width='100%' height={300}>
              <PieChart width={400} height={400}>
                <Pie
                  data={dataPieChart2}
